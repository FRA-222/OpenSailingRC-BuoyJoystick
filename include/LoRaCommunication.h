...
// seul le canal radio change. La fréquence n'est PAS détectable par logiciel,
// c'est donc le mode de communication (CommMode::LORA_920 / LORA_433) qui
// détermine la bande, et le switch M0/M1 du module qui gère sa configuration.
enum class LoRaBand {
    BAND_920,   ///< Module E220-JP    : F = 920.6 + canal * 0.2 MHz (canal 0-30)
    BAND_433    ///< Module E220-400T22D : F = 410.125 + canal * 1 MHz (canal 0-80)
};

// LoRa E220 configuration
// IMPORTANT : le canal doit être IDENTIQUE côté joystick et côté bouée.
#define LORA_CHANNEL_920 0x00       // 920.6 MHz (bande ISM japonaise)
#define LORA_CHANNEL_433 23         // 433.125 MHz (canal par défaut du E220-400)
#define LORA_CHANNEL LORA_CHANNEL_920  // Conservé pour compatibilité (bande 920)
...
public:
     ...
     *
     * Has no effect once begin() has run: the channel is pushed to the module
